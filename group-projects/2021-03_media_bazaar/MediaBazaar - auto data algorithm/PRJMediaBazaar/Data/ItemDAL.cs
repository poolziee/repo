...
namespace PRJMediaBazaar.Data
{
    class ItemDAL : BaseDAL
    {
       ...
       private bool UpdateItemShopQuantity(int itemId, int newInShopAmount)
        ...
        public void InsertNewAutoOrder(Order order, int cashierId, DateTime date)
        {
            try
            {
                // Use current time
                ...
                String sql = "INSERT INTO orders (cashier_id, date, total_price) VALUES(@cashierId, @date, @totalPrice) ;";
                String[] parameters = new String[] { cashierId.ToString(), date.Date.ToString(format), order.GetTotalPrice().ToString() };
                if (executeNonQuery(sql, parameters) != null)
                {
                    CloseConnection();
                    int orderId = LastOrderId();
                    foreach (Item i in order.BoughtItems)
                    {
                        ...
                        //move item to  shop if necesarry
                        int movingAmount = i.GetMovingAmount();
                        if(i.RoomInStorage - movingAmount > 0)
                        {
                          
                            int newInShopAmount = i.InShopAmount + movingAmount;
                            int newInStorageAmount = i.InStorageAmount - movingAmount;
                            i.InShopAmount = newInShopAmount;
                            i.InStorageAmount = newInStorageAmount;
                            i.AmountToRestock = i.GetMaxFreeSpaceInStorage();

                            //add it to restock list if needed
                            if (i.InStorageAmount <= i.MinimumAmountInStock && i.Restock_State != "stocker" &&  i.AmountToRestock >= 0)
                            {
                                i.Restock_State = "stocker";
                            }
                        }
                    }
        ...
        public void InsertNewAutoRestock(Restock restock, int managerId, DateTime date)
        {
            try
            {
                string format = "yyyy-MM-dd";
                String sql = "INSERT INTO restocks (manager_id, date, total_price) VALUES(@managerId, @date, @totalPrice) ;";
                String[] parameters = new String[] { managerId.ToString(), date.Date.ToString(format), restock.GetTotalCostAuto().ToString() };
                if (executeNonQuery(sql, parameters) != null)
                {
                    CloseConnection();
                    int restockId = LastRestockId();
                    foreach (Item i in restock.GetItemsForAutoRestock())
                    {
                        InsertRestockItem(restockId, i.ID, i.AmountToRestock);
                        int newInStorage = i.InStorageAmount + i.AmountToRestock;
                        i.Restock_State = "stable";
